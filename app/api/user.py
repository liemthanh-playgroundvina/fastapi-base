...
def create(user_data: UserCreateRequest) -> Any:
    ...
    try:
        ...
        user_res = UserService().create_user(user_data)
        return DataResponse().success_response(data=user_res)
    except Exception as e:
    ...
    try:
        if user_data.username is not None:
            exist_user = db.session.query(User).filter(
                User.username == user_data.username, User.id != current_user.id).first()
            if exist_user:
                raise Exception('[username] already exists')
